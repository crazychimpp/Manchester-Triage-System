...
import { apiEnabled, requestReview } from '../lib/api.js';
...
export default function GemmaPanel({ patient, engine, onStatus, onResult, result }) {
  ...
  async function ask(transport) {
    ...
    try {
      // With a backend the call goes through the server: it holds the model
      // host, enforces the timeout, and stores the prompt with the response so
      // the review can be reconstructed exactly if the priority is ever
      // questioned. Without one, the browser calls the model directly as before.
      if (apiEnabled) {
        const out = await requestReview(buildPrompt(patient, engine), {
          transport,
          signal: abort.current.signal,
        });
        if (out.failed) throw new Error(out.error || 'the model host did not answer');
        onResult({ ...out.review, reviewId: out.id });
        onStatus('idle');
        return;
      }
